...
number1 = 300; // number1 에 300 재대입(기존 10 값에 덮어쓰기)
...
// 이전에 선언 해놓은 변수를 쓸 수 없게되는 문제가 발생함.

// -------------------------------------------------------------------------------

// 2.  let(변수, var 의 변수명 중복 문제 해결)
let number3 = 25;
// let number3 = 500; // 변수명 중복 불가

number3 = 500; // 기존 변수 number3 에 새로운 값 재할당 가능
...
console.log(foo1);// 전역 변수이기 때문에(블록과 관계 없음) 접근 가능
console.log(foo2);// 전역 변수이기 때문에(블록과 관계 없음) 접근 가능
// console.log(num1); //Uncaught ReferenceError: num1 is not defined
// console.log(num2); // Uncaught ReferenceError: num2 is not defined
// -> 둘 다 블록레벨 스코프 이기 때문에 블록 밖에서 접근할 수 없음.

// ---------------------------------------------------------------------

// 함수 레벨 스코프 (VAR)
// var 키워드로 선언된 변수는 함수 내 어디서든 접근할 수 있음.
// if, for 등의 일반 블록은 무시하고, var 로 선언된 변수가 있는
// 함수 전체에서 유효함.
...
console.log(test2);
// test2 도 접근 가능
// -> 왜? 함수 레벨 스코프인 var 는 일반블록은 무시하기 때문에.
...
// 1. var 의 호이스팅
// var 로 선언된 변수는 선언만 해당 스코프의 최상단으로
// 호이스팅 됨. 초기화는 원래 코드 위치에서 수행된다.

// 에러가 안나고 undifined 가 찍힘
...
// var 와 달리 let/const/ 로 선언된 변수는 초기화가 이루어지기 전에는
// 사용될 수 없음.

// TDZ(Temporal Dead Zone)
// 시간적으로 죽은 구역..
// 코드 실행 과정에서 특정 시점가찌 변수가 접근 불가능한 상태를 의미함
// Javascript 에서 let/const 키워드로 선언된 변수가
// 초기화 되기 전까지 접근할 수 없는 구간을 말함.

// let x; (내부적으로 선언은 끌어올려진 상태)
// -> X 의 선언이 호이스팅됨
// 그러나 TDZ 에 놓여 초기화 전까지 접근 불가
// -> 초기화기 이루어 지기 전까지 접근 불가
// 이 기간동안 변수에 접근 하려고 하면
// Uncaught ReferenceError: Cannot access 'x' before initialization 라는 에러 발생
...
// undefinde : 정의되지 않은 변수 / 값이 dkwlr eodlqehlwl dksgdkTek.
let undef; // 변수 선언
...
console.log("heigt : ", height, typeof height);
console.log("eyesight : ", eyesight, typeof eyesight);

// boolean (논리값 true / flase)
const isTrue = true;
const isFalse = false;
