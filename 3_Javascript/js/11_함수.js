...
// btn1 클릭시 input1 의 값을 읽어와 print1() 이라는 함수 호출
...
// 숫자 --> 원시 타입
// arr 은 배열 == 즉 참조형
// arrayTest()함수에 전달인자로 arr 을 전달하면
// arr 에 저장된 주소만 보내서 함수를 수행(얕은 복사)
...
document.querySelector("#btn2c").addEventListener("click", function(){
 ...
 // print2 함수에 전달 인자로 sumFn을 전달
 print2(sumFn);
})

//화살표 함수

// 클래스가 arrow 인 요소를 모두 얻어옴(배열)
...
// return 1줄만 작성된 경우
const twoNumberPlus = (othetFn) => { // 요새 많이 쓰는 방식
 const input1 = Number(prompt("첫 번째 값"));
 const input2 = Number(prompt("두 번째 값"));

 alert(othetFn(input1, input2));
}
...
printObject( (name, age) => {
  ...
  // {"name" : "홍길동", "age" : 20};
  // js 객체 object 타입 ! => return 1줄이어도 생략 불가
} )
});
